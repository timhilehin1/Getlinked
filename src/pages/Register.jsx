	...
	const SelectGroup = ({ label, options, value, required, name }) => {
		return (
			<div className='flex flex-col'>
				...
				>
					{options?.map((option) => (
						<option className='bg-[#ffffff08]' value={option}>
							{option}
						</option>
					))}
				</select>
		...
		<div className='px-6 md:px-0'>
			...
			<section className='py-20  mx-auto container md:grid md:grid-cols-2 items-center'>
				...
				<form className='questions p-8 flex flex-col gap-8'>
                    ...
                    <button className="w-full rounded p-3 outline-none text-white bg-gradient-to-r from-purple-600 via-purple-400 to-pink-400 h-3.3rem">Register Now</button>
				</form>
			</section>
		</div>
	);
